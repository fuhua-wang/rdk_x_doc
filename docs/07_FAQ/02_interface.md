...
3.  **串口终端软件参数配置：**
    ...
    * 串口号 (COM Port)：确保选择了连接USB转串口模块后在电脑设备管理器中识别到的正确串口号。
    ![串口终端参数配置示例](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/08_07_07_FAQ/image/interface/image-20221124200013163.png)
...
5.  **参考官方文档：** 详细的网络配置步骤和故障排除方法，请参考官方文档中关于“网络配置”的章节。
    ...
    [RDK X3/X5 网络配置](../03_03_System_configuration/network_blueteeth)
    </TabItem>
    <TabItem value="rdk_s100" label="rdk_s100">
    [RDK S100 网络配置](/rdk_s/03_03_System_configuration/network_bluetooth)
    </TabItem>
    </Tabs>
...
* **提示 `Connection timed out`：**
    ...
    * **参考：** [SSH登录](../02_02_02_Quick_start/04_02_02_02_Quick_start/04_remote_login.md)章节 (请将链接替换为实际有效的文档路径)。
...
* **提示 `Authentication failed` 或 `Permission denied, please try again.`：**
    ...
    * **排查：**
    ...
    ![SSH认证失败示例](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/08_07_07_FAQ/image/interface/image-20221124201544978.png)
...
1.  **摄像头连接方式错误：**
    ...
    * **参考文档：** 仔细查阅您所使用的RDK板卡型号对应的硬件手册或快速入门指南中关于“MIPI摄像头”连接的章节，确认连接细节。
        ...
        [RDK X3 MIPI摄像头](../02_02_02_Quick_start/01_hardware_introduction/01_rdk_x3.md)
        </TabItem>
        <TabItem value="rdk_x5" label="rdk_x5">
        [RDK X5 MIPI摄像头](../02_02_02_Quick_start/01_hardware_introduction/02_rdk_x5.md)
        </TabItem>
        <TabItem value="rdk_s100" label="rdk_s100">
        [RDK S100 MIPI摄像头](/rdk_s/02_02_Quick_start/hardware_introduction/rdk_s100_camera_expansion_board)
        </TabItem>
        </Tabs>
2.  **禁止带电插拔：**
...
* **查询方法：**
    1.  **官方文档：** 最准确的信息来源是查阅您所使用的具体RDK板卡型号和系统版本的官方《用户手册》或《硬件规格说明书》中关于“[HDMI接口](../02_02_02_Quick_start/01_hardware_introduction/01_rdk_x3.md)”或显示子系统的章节 (请将链接替换为实际有效的文档路径)。
    2.  **系统内查询（如果已连接显示器）：**
